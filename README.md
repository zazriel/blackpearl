...
A sleek, modern desktop chat application built with **Python**, powered by **PubNub's** global real-time network and styled with the **ModernTkinter** widget library. This project focuses on secure, real-time communication with a scalable architecture and a clean user experience.
...
* 🖥️ **Modern UI**
  Clean, contemporary interface with dark/light mode support via `ModernTkinter`.
...
* 🟢 **User Presence (Planned/Optional)**
...
* **ModernTkinter** – Modern UI components and theming

---

## ⚙️ How It Works

1. A secure token is generated using a backend script (`GP.py`).
...
cd blackpearl
pip install pubnub moderntkinter
```
...
```bash
python blackpearl_backendV1.py
```
...
* ⏳ Tokens expire after a set TTL — regenerate when needed.
* 🔁 The `uuid/username` must **exactly match** the one used during token generation.
